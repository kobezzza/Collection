/*!
 * Collection v6.6.15 (node)
 * https://github.com/kobezzza/Collection
 *
 * Released under the MIT license
 * https://github.com/kobezzza/Collection/blob/master/LICENSE
 *
 * Date: 'Thu, 22 Feb 2018 09:56:32 GMT
 */
 ...
 * @const
 */
Collection.prototype.VERSION = [6, 6, 15];
	...
	switch (p.type) {
		...
		case 'asyncIterator':
			...
			if (p.type === 'asyncIterator') {
				asyncIterator$$1 = ws(_templateObject33);
			}
			...
			if (p.reverse) {
				...
				if (needParallel) {
					iFn += ws(_templateObject34, parallelFn, yielder);
				}
...
Collection.prototype.forEach = function (opt_cb, opt_params) {
	...
	if (asyncTypes[type]) {
		...
		var on = 'add' + (isStream$$1 ? '' : 'Event') + 'Listener',
		    off = 'remove' + (isStream$$1 ? '' : 'Event') + 'Listener',
