 ...
 *
 * Date: 'Mon, 04 Sep 2017 09:27:26 GMT
 */
 ...
 * @const
 */
Collection.prototype.VERSION = [6, 0, 3];
...
function compileCycle(key, p) {
	...
	if (needCtx) {
		iFn += ws(_templateObject3, p.mult, p.startIndex, p.endIndex, p.from, p.count, p.live, p.reverse, p.withDescriptor, p.notOwn, p.inverseFilter, p.type, p.async, p.thread, p.thread, p.priority, p.length, cantModI, cantModI);
	...
	if (p.thread) {
		threadStart = ws(_templateObject7);

		threadEnd = ws(_templateObject8);
	}

	if (isAsync) {
		getEl = ws(_templateObject9);
	}
	...
	switch (p.type) {
		case 'array':
			...
			if ((p.reverse || !p.live) && (startIndex || endIndex)) {
				iFn += ws(_templateObject11, startIndex, endIndex || 'data.length');
			}

			if (!p.reverse && p.live) {
				iFn += ws(_templateObject12, startIndex - 1);

				if (startIndex) {
					iFn += ws(_templateObject13, startIndex);
				}

				if (endIndex) {
					iFn += ws(_templateObject14, endIndex);
				}
			} else {
				iFn += ws(_templateObject15, startIndex);
			}
		...
		case 'object':
			...
			if (p.reverse || OBJECT_KEYS_NATIVE_SUPPORT && !p.notOwn) {
				...
				iFn += ws(_templateObject19, startIndex);
			} else {
				iFn += 'for (key in data) {';
		...
		case 'iterator':
			...
			if (p.reverse) {
				...
				iFn += ws(_templateObject25, defArgs ? 'key = tmpArray[n];' : '', startIndex);
			} else {
				gen();
	...
	iFn += threadStart;
	if (p.count) {
		iFn += ws(_templateObject27, p.count);
	}
	...
	var yielder = ws(_templateObject33);
	...
	iFn += ws(_templateObject35, threadEnd);
...
Collection.prototype.forEach = function (cb, opt_params) {
	...
	var key = [type, cbArgs, filters.length, filterArgs, p.length, p.async, p.thread, p.withDescriptor, p.notOwn, p.live, p.inverseFilter, p.reverse, p.mult, p.count, p.from, p.startIndex, p.endIndex].join();
