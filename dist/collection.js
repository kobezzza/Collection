...
function compileCycle(key, p) {
	...
	if (needCtx) {
		iFn += ws(_templateObject11, p.mult, p.live, p.reverse, p.withDescriptor, p.notOwn, p.inverseFilter, p.type, p.async, p.thread, p.thread, p.priority, p.length, cantModI, cantModI);
	...
	if (p.thread) {
		threadStart = ws(_templateObject14);

		threadEnd = ws(_templateObject15);
	}

	if (isAsync) {
		getEl = ws(_templateObject16);
	}
	...
	switch (p.type) {
		...
		case 'iterator':
			...
			if (p.reverse) {
				gen();
				iFn += ws(_templateObject32, threadStart, getEl, threadEnd);

				if (p.startIndex || p.endIndex) {
					iFn += 'tmpArray = tmpArray.slice(startIndex, endIndex || tmpArray.length);';
				}

				iFn += ws(_templateObject33, defArgs ? 'key = tmpArray[n];' : '');
			} else {
				gen();

				iFn += ws(_templateObject34, defArgs ? 'key = \'value\' in key ? key.value : key;' : '');
	...
	if (isAsync) {
		tmp += ws(_templateObject38);
	}

	if (!needWrapper && p.from) {
		iFn += ws(_templateObject39, tmp);
	} else {
		iFn += tmp;
	}
