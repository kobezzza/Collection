 ...
 *
 * Date: 'Wed, 30 Aug 2017 09:59:29 GMT
 */
...
function compileCycle(key, p) {
	...
	var iFn = ws(_templateObject, p.mult, p.startIndex, p.endIndex, p.from, p.count, p.live, p.reverse, p.withDescriptor, p.notOwn, p.inverseFilter, p.type, p.async, p.thread, p.thread, p.priority, p.length);

	if (isAsync) {
		iFn += ws(_templateObject2);
	}

	iFn += ws(_templateObject3, !isAsync, !isAsync, !isAsync, !isAsync, !isAsync, !isAsync, cantModI, cantModI);
	...
	switch (p.type) {
		case 'array':
			...
			if ((p.reverse || !p.live) && (startIndex || endIndex)) {
				iFn += ws(_templateObject9, startIndex, endIndex || 'data.length');
			}

			if (!p.reverse && p.live) {
				iFn += ws(_templateObject10, startIndex - 1);

				if (startIndex) {
					iFn += ws(_templateObject11, startIndex);
				}

				if (endIndex) {
					iFn += ws(_templateObject12, endIndex);
				}
			} else {
		...
		case 'object':
			...
			if (p.reverse || OBJECT_KEYS_NATIVE_SUPPORT && !p.notOwn) {
				...
				iFn += ws(_templateObject17, startIndex);
			} else {
				iFn += 'for (key in data) {';
		...
		case 'iterator':
			...
			if (p.reverse) {
				gen();
				iFn += ws(_templateObject22, threadStart, getEl, threadEnd);

				if (startIndex || endIndex) {
					iFn += 'tmpArray = tmpArray.slice(' + startIndex + ', ' + (endIndex || 'tmpArray.length') + ');';
				}

				iFn += ws(_templateObject23, defArgs ? 'key = tmpArray[n];' : '', startIndex);
			} else {
				gen();

				iFn += ws(_templateObject24, defArgs ? 'key = \'value\' in key ? key.value : key;' : '');
	...
	iFn += threadStart;
	if (p.count) {
		iFn += ws(_templateObject25, p.count);
	}
