 ...
 *
 * Date: 'Mon, 04 Sep 2017 09:44:04 GMT
 */
 ...
 * @const
 */
Collection.prototype.VERSION = [6, 0, 4];
...
function compileCycle(key, p) {
	...
	switch (p.type) {
		...
		case 'object':
			...
			if (p.reverse || OBJECT_KEYS_NATIVE_SUPPORT && !p.notOwn) {
				...
				iFn += ws(_templateObject20, startIndex);
			} else {
				iFn += 'for (key in data) {';
		...
		case 'iterator':
			var gen = function () {
				if (isMapSet) {
					...
					iFn += ws(_templateObject24);
				}
			};

			if (p.reverse) {
				gen();
				iFn += ws(_templateObject25, threadStart, getEl, threadEnd);

				if (startIndex || endIndex) {
					iFn += 'tmpArray = tmpArray.slice(' + startIndex + ', ' + (endIndex || 'tmpArray.length') + ');';
				}

				iFn += ws(_templateObject26, defArgs ? 'key = tmpArray[n];' : '', startIndex);
			} else {
				gen();

				iFn += ws(_templateObject27, defArgs ? 'key = \'value\' in key ? key.value : key;' : '');
	...
	iFn += threadStart;
	if (p.count) {
		iFn += ws(_templateObject28, p.count);
	}

	iFn += getEl;
	if (p.filter.length) {
		for (var _i = 0; _i < p.filter.length; _i++) {
			iFn += ws(_templateObject29, _i, filterArgs[_i]);

			if (isAsync) {
				iFn += ws(_templateObject30);
			}

			iFn += ws(_templateObject31, p.inverseFilter ? '!' : '');
		}
	...
	var yielder = ws(_templateObject34);
	...
	iFn += ws(_templateObject36, threadEnd);
