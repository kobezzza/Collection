...
function compileCycle(key, p) {
	...
	if (needWrapper) {
		...
		if (p.count) {
			fnCountHelper += ws(_templateObject8);
		}

		if (isAsync) {
			iFn += ws(_templateObject9, fLength ? resolveFilterVal : '', fnCountHelper, cbArgs, fnCountHelper, cbArgs);

			if (needParallel) {
				var fn = p.parallel ? 'wait' : 'race';

				iFn += ws(_templateObject10, fn, fn);
			} else {
				iFn += 'return res;';
			}
		} else {
			iFn += ws(_templateObject11, fnCountHelper, cbArgs);
		}
	...
	if (needCtx) {
		iFn += ws(_templateObject12, p.mult, p.live, p.reverse, p.withDescriptor, p.notOwn, p.inverseFilter, p.type, p.async, p.thread, p.thread, p.priority, p.length, cantModI, cantModI);
	...
	if (p.thread) {
		threadStart = ws(_templateObject15);

		threadEnd = ws(_templateObject16);
	}

	if (isAsync) {
		getEl = ws(_templateObject17);
	}
	...
	switch (p.type) {
		...
		case 'iterator':
			...
			if (p.reverse) {
				gen();
				iFn += ws(_templateObject33, threadStart, getEl, threadEnd);

				if (p.startIndex || p.endIndex) {
					iFn += 'tmpArray = tmpArray.slice(startIndex, endIndex || tmpArray.length);';
				}

				iFn += ws(_templateObject34, defArgs ? 'key = tmpArray[n];' : '');
			} else {
				gen();

				iFn += ws(_templateObject35, defArgs ? 'key = \'value\' in key ? key.value : key;' : '');
		...
		if (fLength) {
			if (fLength < 5) {
				for (var _i2 = 0; _i2 < fLength; _i2++) {
					iFn += ws(_templateObject37, !_i2 ? 'f === undefined || ' : '', _i2, filterArgs[_i2], resolveFilterVal);
				}
			} else {
				iFn += ws(_templateObject38, callCycleFilter, resolveFilterVal);
			}
	...
	if (isAsync) {
		tmp += ws(_templateObject39);
	}

	if (!needWrapper && p.from) {
		iFn += ws(_templateObject40, tmp);
	} else {
		iFn += tmp;
	}
	...
	var yielder = ws(_templateObject41);
